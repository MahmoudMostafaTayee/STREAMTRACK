...
\def\mycopyrightnotice{%
  {\footnotesize XXX-X-XXXX-XXXX-X/XX/\$XX.00~\copyright~20XX IEEE\hfill}% <--- Change here (camera-ready)
...
\begin{figure}[t!]
    \centering
    \includegraphics[width=0.95\linewidth]{Evidence/fig_stage_speedup.png}
...
We benchmarked windowed HAC~\cite{mullner2011modern} against specialized stream-clustering algorithms (CluStream~\cite{aggarwal2003framework} and ClusTree~\cite{kranen2011clustree} via MOA~\cite{bifet2010moa}). HAC achieves the lowest latency ($82.4$~ms) and 100\% identity precision (fraction of tracklets matching their reference identity) at MCOT window sizes ($N=50$--$500$), whereas CluStream's micro-clusters add high initialization overhead ($3{,}854.2$~ms; 82.5\%) and blur appearance features; ClusTree is comparable ($2{,}510.8$~ms; 78.1\%), as summarized in Table~\ref{tab:clustering_comparison}.
...
The timing decomposition shows that reconfiguration overhead is dominated by query compilation, not state: the initial deployment of the eight aggregation queries took 1{,}759~ms (including JVM warm-up and first EPL compilation), while subsequent single-group topology merges completed in 170--191~ms each, all far below the 3-second synchronisation window.

The same event-driven pattern extends beyond topology to the algorithmic parameters of the tracker. \texttt{simTh}, \texttt{epsilonMcpt}, and \texttt{shortTrackTh} are stored in an \texttt{MCPTConfigTable} and updated reactively by streaming \texttt{MCPTConfig} events; the MCPT kernel re-reads fresh values on every window call. To demonstrate this, we injected three parameter changes at frame 270---tightening the similarity threshold \texttt{simTh} from 0.75 to 0.85, reducing the clustering radius \texttt{epsilonMcpt} from 0.37 to 0.30, and raising the short-track pruning threshold \texttt{shortTrackTh} from 0 to 1---all applied with no restart, no code modification, and no loss of accumulated tracking state, with the new values effective from the very next synchronisation window. This is a capability demonstration of runtime parameter adaptation rather than an accuracy benchmark target; its purpose is to show that the streams-and-tables substrate supports operational tuning without downtime, complementing the latency results measured for topology changes.
