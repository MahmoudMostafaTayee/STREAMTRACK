...
We benchmarked windowed HAC against specialized stream-clustering algorithms (CluStream~\cite{aggarwal2003framework} and ClusTree~\cite{kranen2011clustree} via MOA). HAC achieves the lowest latency ($82.4$~ms) and 100\% identity precision (fraction of tracklets matching their reference identity) at MCOT window sizes ($N=50$--$500$), whereas CluStream's micro-clusters add high initialization overhead ($3{,}854.2$~ms; 82.5\%) and blur appearance features; ClusTree is comparable ($2{,}510.8$~ms; 78.1\%).
...
Short-term intra-camera occlusions are cushioned by the 3-second SCT window and keypoint filters reject occluded boxes. Long-term cross-camera occlusions rely on \texttt{GlobalIDTable} persistence (un-matched identities survive up to 120~s and re-associate on reappearance) and \texttt{TrackExpiredEvent}-driven re-association; predictive interpolation through occluded zones via CEP patterns is future work.

\subsection{Operational Advantages}
Queryable state lets operators audit identity assignments via standard SQL in real time, and per-camera SCT can run on edge devices emitting compact \texttt{SingleCameraResult} events over low-bandwidth links to a centralized coordinator ($9.15$~ms per group).
