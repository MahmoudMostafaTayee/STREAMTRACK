...
Implementing MCOT via SQL/EPL on a Complex Event Processing (CEP) engine like Esper~\cite{espertech2024} provides several operational advantages: (i) \textbf{Inspectability:} Developers can query intermediate state tables (e.g., \texttt{GlobalIDTable}) at runtime to debug association failures. (ii) \textbf{Modularity:} New operators can be added by simply deploying new EPL statements. (iii) \textbf{Memory Safe:} The engine gracefully handles complex event lifecycle rules like \texttt{TrackExpiredEvent}, ensuring deterministic garbage collection of heavy vision tensors. (iv) \textbf{Performance:} The engine's efficient in-memory event processing yields a \textbf{156$\times$} average speedup in similarity generation on the NVIDIA SmartSpaces dataset. By maintaining all state and features directly in the CEP engine's memory space, we eliminate the costly file serialization and disk I/O overhead that characterizes the batch baseline.
...
The NVIDIA AI City Challenge~\cite{wang2024aicity} has become the primary benchmark for multi-camera people tracking (MCPT), catalyzing a series of high-performing systems. In the 2024 edition (Track~1), Yoshida et al.~\cite{yoshida2024overlap} proposed overlap suppression clustering for offline MCPT, achieving the top HOTA score by merging duplicate tracklets across overlapping fields of view---our system builds upon their pipeline as the baseline. Kim et al.~\cite{kim2024cluster} introduced cluster self-refinement for enhanced online tracking via iterative identity correction. Xie et al.~\cite{xie2024robust} presented a robust online system combining geometric consistency with state-aware Re-ID correction, securing second place. Specker~\cite{specker2024ocmctrack} proposed OCMCTrack, a corrective matching cascade that dynamically re-evaluates track assignments to reduce false-positive associations. Yang et al.~\cite{yang2024online} addressed online tracking in extremely long video sequences, introducing evaluation methods for sustained identity consistency. While these systems advance the state of the art in detection, Re-ID, and clustering strategies, they remain architecturally coupled to monolithic batch or batch-window processing paradigms. None address runtime reconfigurability, intermediate-state inspectability, or topology management as first-class system concerns.

\paragraph{Online vs.\ Offline Tracking and Scalability.}
Multi-camera tracking systems are broadly classified as \emph{offline} (batch) or \emph{online} (incremental). Offline methods~\cite{yoshida2024overlap} leverage global optimization over entire video sequences, exploiting future frames for higher accuracy but incurring latency that precludes real-time deployment. Online methods~\cite{kim2024cluster,xie2024robust,specker2024ocmctrack} process observations causally and are suitable for live settings, yet even these ``online'' solutions typically rely on internal batch windows for cross-camera association, accumulating detections over fixed intervals before performing global clustering. Critically, \emph{scalability} in the MCPT literature is almost exclusively treated as a computer-vision concern (handling more detections, denser crowds) rather than a \emph{systems} concern (adding cameras, reconfiguring topology, maintaining state under growth). Our work is orthogonal to these accuracy-centric advances: we take an existing offline pipeline and re-architect it as a streaming program, preserving its functional output while gaining systems-level properties---scalability via topology-aware partitioning, inspectability via queryable state tables, and reconfigurability via live topology updates.

\paragraph{Industrial Video Analytics Frameworks.}
NVIDIA DeepStream~\cite{nvidia2024deepstream} is the dominant industrial SDK for GPU-accelerated video analytics. Built on GStreamer, it provides a modular pipeline of hardware-accelerated plugins for decoding, inference, and single-camera tracking (NvDCF, DeepSORT, IOU). However, its native trackers operate within a single camera stream; cross-camera identity association requires an external microservice layer. NVIDIA Metropolis~\cite{nvidia2025metropolis} addresses this gap with a Kubernetes-based reference architecture that decomposes multi-camera tracking into containerized microservices communicating via message brokers. While Metropolis provides a scalable deployment model, it remains a heavyweight, opaque infrastructure: association logic is embedded in compiled services rather than expressed as declarative queries, intermediate tracking state is not directly queryable for debugging, and reconfiguring the camera network (adding or removing cameras, changing neighborhood links) typically requires container re-deployment or manual API calls rather than a single streaming event. In contrast, \textbf{STREAMTRACK} expresses association as parameterized SQL/EPL joins over a relational topology table, enabling zero-downtime reconfiguration, SQL-based state inspection, and a lightweight single-process deployment.

\paragraph{Stream Processing Foundations and CEP.}
Our architectural approach draws from the streams-and-tables duality formalized by Sax et al.~\cite{sax2018streams}, which established that streams and tables are dual representations of the same underlying data---a stream is a changelog of a table, and a table is a materialized view of a stream. Apache Kafka~\cite{kreps2011kafka} and its Kafka Streams library operationalized this duality via KTables and changelog topics, enabling stateful stream processing at scale, though without native support for the windowed join semantics required by vision workloads. Apache Flink~\cite{carbone2015flink} provides a more expressive model with native Complex Event Processing (CEP) pattern detection, event-time watermarks, and exactly-once guarantees, but introduces significant infrastructure complexity (cluster deployment, checkpoint management). Our implementation uses Esper~\cite{espertech2024}, a mature CEP engine that combines SQL-like continuous queries (EPL) with full relational table semantics in an embeddable, single-process runtime. The closest prior work applying CEP to video analytics is VidCEP~\cite{yadav2019vidcep}, which introduced a graph-based Video Event Knowledge Graph for detecting spatiotemporal patterns (e.g., loitering, trajectory anomalies) in video streams. However, VidCEP targets \emph{event and anomaly detection}---recognizing high-level behavioural patterns from atomic detections---rather than the structurally different problem of \emph{multi-camera identity tracking}, which requires continuous maintenance of mutable identity state, cross-camera feature association, and topology-aware join planning. To our knowledge, no prior work has applied the streams-and-tables paradigm to the core MCOT pipeline itself.

\paragraph{Summary and Gap.}
The literature reveals a clear gap at the intersection of multi-camera tracking and stream processing systems. CV-oriented works~\cite{yoshida2024overlap,kim2024cluster,xie2024robust,specker2024ocmctrack} focus on improving tracking accuracy but treat the processing architecture as a fixed implementation detail. Industrial frameworks~\cite{nvidia2024deepstream,nvidia2025metropolis} provide scalable deployment but lack declarative association logic, queryable state, and zero-downtime reconfigurability. Stream processing systems~\cite{sax2018streams,carbone2015flink,kreps2011kafka} provide the theoretical and practical foundations for continuous data processing but have not been applied to MCOT. \textbf{STREAMTRACK} bridges this gap by formulating multi-camera tracking as a relational streaming program in which identity is a continuously maintained table, association is a parameterized stream join, and the camera topology is a reconfigurable state relation.
