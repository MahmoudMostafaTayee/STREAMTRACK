...
\def\mycopyrightnotice{%
  {\footnotesize 979-8-XXXX-XXXX-X/27/\$31.00~\copyright~2027 IEEE\hfill}% <--- REPLACE with the IEEE ISBN assigned at acceptance
...
% ---------------------------------------------------------------------------
% Content-support packages required by the manuscript body
% (kept from the original draft; not part of the ACDSA template itself)
% ---------------------------------------------------------------------------
\usepackage{booktabs}
\usepackage{multirow}
\usepackage{url}
...
\begin{document}
\title{\vspace*{1cm} Streams-and-Tables for Scalable Multi-Camera Object Tracking}

% =========================================================================
% AUTHOR BLOCK (CAMERA-READY)
% =========================================================================
\author{\IEEEauthorblockN{Mahmoud Tayee}
\IEEEauthorblockA{\textit{Faculty of Engineering and Applied Sciences} \\
\textit{Nile University}, Giza, Egypt \\
\texttt{m.mostafa2118@nu.edu.eg}}
\and
...
Continuous Query Language (CQL) models~\cite{sax2018streams,DBLP:journals/vldb/ArasuBW06} unify streaming and relational data across four operator families: stream-to-relation (S2R; temporal windows such as \texttt{win:time}), relation-to-relation (R2R; SQL over materialised tables), stream/relation-to-relation (S/R2R; upserts into persistent tables), and relation-to-stream (R2S; timer-driven evictions).

While distributed engines (e.g., Apache Flink~\cite{carbone2015flink}, Kafka Streams~\cite{kreps2011kafka}) excel at data-parallel key-value tasks, transferring high-dimensional appearance vectors across network sockets incurs severe RPC serialisation overhead. We select Esper~\cite{espertech2024}, a mature CEP engine providing full CQL semantics in an embeddable, single-process JVM runtime, enabling zero-copy sharing of feature vectors between vision kernels and relational state tables.
...
2) \textbf{Windowed Single-Camera Tracking (S2R):} Local frame detections are grouped into tracklets over a 3-second tumbling window (90 frames at 30~fps, \texttt{win:length\_batch(90)}), emitting structured single-camera result events.
...
To track objects across isolated, non-overlapping cameras without identity fragmentation, the topology relation maintains overlapping edges ($E_{\text{overlap}}$) and directed transition edges ($E_{\text{transition}}$). Global association allows comparing a historical track $T_H$ (last seen at camera $c_{\mathrm{last}}$) with a current cluster $C_{\mathrm{curr}}$ only if $c_{\mathrm{last}}$ belongs to $C_{\mathrm{curr}}$ or shares an overlap/transition edge with some camera in $C_{\mathrm{curr}}$---restricting matching to physically valid travel routes and preventing false matches across distant areas (Fig.~\ref{fig:transition_flow}).

\begin{figure}[t!]
    \centering
    \includegraphics[width=0.72\linewidth]{Evidence/map_transition_flow.png}
    \caption{Illustrative transition topology mapping for isolated (non-overlapping) cameras. A person leaving the field of view of \texttt{Camera 12} can transition to \texttt{Camera 02}, \texttt{Camera 13}, or \texttt{Camera 17}. Transitional gating restricts Re-ID candidate matching to these paths, preventing identity fragmentation across non-overlapping zones.}
...
To verify that our re-engineering preserves accuracy, we conducted a \textbf{five-stage parity evaluation} against a JVM-native batch baseline. As the cited OSC pipeline~\cite{yoshida2024overlap} is Python, we ported it to this functionally equivalent JVM baseline, validated on identical inputs (ARI = NMI = 1.0000; similarity-matrix MAD $1.63\times10^{-8}$). Parity and port-validation metrics are computed by the comparison scripts in \texttt{scripts/} over per-window pipeline dumps; since parity holds at every stage, downstream accuracy metrics (IDF1, HOTA) are preserved by construction:
\begin{enumerate}
    \item \textbf{Numerical Parity:} Cosine matrices match across RAW, ZEROED, and REPLACED stages ($\text{MAD} = 1.63 \times 10^{-8}$).
    \item \textbf{Structural Parity:} Adjusted Rand Index ($\text{ARI}$) and Normalized Mutual Information ($\text{NMI}$) achieve \textbf{1.0000} across all 175 commonly-aligned tracklets (14/14 cluster groups match perfectly). The batch pipeline adds one extra representative node (serial 1018)---a pre-clustering artefact with no effect on the shared node set.
    ...
    \item \textbf{Topology Parity:} The inter-camera association-count matrix converges to an identical $12\times 12$ shape (12 representative tracklets in both pipelines).
...
\begin{tabular}{lrrrc}
\toprule
\textbf{Pipeline Stage} & \textbf{Batch (ms, 25-window total)} & \textbf{Streaming (ms/window, mean $\pm$ std)} & \textbf{Speedup} & \textbf{Parity Metric} \\
\midrule
...
Re-ID Similarity Matrix & 65.08 & $3.67 \pm 8.28$ & 0.7$\times$ & MAD $= 1.63\times 10^{-8}$ \\
World Coord. Measure & 8.65 & $1.25 \pm 0.79$ & 0.3$\times$ & Exact Match \\
...
\vspace{2pt}
\parbox{\linewidth}{\raggedright\footnotesize Note: batch figures are cumulative over 25 windows; streaming figures are per-window means; speedup $=$ batch $\div$ (streaming $\times$ 25). Sub-1.0$\times$ stages recompute per window with no amortization benefit; totals include unlisted orchestration overhead.}
\end{table}
...
The batch baseline re-reads and re-processes full historical feature archives from disk each window ($O(N^2)$ growth), requiring 31,620~ms total; our system maintains state incrementally in relational tables, reducing cumulative execution to 3,232~ms---a \textbf{9.8$\times$ end-to-end speedup} (Fig.~\ref{fig:cumulative_mcpt}), with \textbf{12.0$\times$} in representative selection and \textbf{9.5$\times$} in HAC clustering. The Re-ID similarity matrix and world-coordinate measure stages show no amortization benefit ($0.7\times$ and $0.3\times$): both pipelines recompute these structures per window over the maintained state, whereas the dominant stages (representative selection, HAC clustering) reuse incremental state. Fig.~\ref{fig:stage_speedup} contrasts the per-stage batch-versus-streaming costs directly.

\begin{figure}[t!]
    \centering
    \includegraphics[width=0.85\linewidth]{Evidence/fig_stage_speedup.png}
    \caption{Stage-by-stage comparison of batch (cumulative over 25 windows) versus incremental streaming (per-window mean). The dominant stages---representative selection and HAC clustering---benefit most from incremental state maintenance (12.0$\times$ and 9.5$\times$), while per-window recomputation stages show near parity.}
    \label{fig:stage_speedup}
\end{figure}

\subsection{RQ2: Clustering Operator Choice and Topological Optimization}
We benchmarked windowed HAC~\cite{mullner2011modern} against specialized stream-clustering algorithms (CluStream~\cite{aggarwal2003framework} and ClusTree~\cite{kranen2011clustree} via MOA~\cite{bifet2010moa}). HAC achieves the lowest latency ($82.4$~ms) and 100\% identity precision at MCOT window sizes ($N=50$--$500$), whereas CluStream's micro-clusters add high initialization overhead ($3{,}854.2$~ms; 82.5\%) and blur appearance features; ClusTree is comparable ($2{,}510.8$~ms; 78.1\%), as summarized in Table~\ref{tab:clustering_comparison}.
...
\begin{tabular}{lrrr}
\toprule
\textbf{Algorithm} & \textbf{Avg. Latency (ms)} & \textbf{ID Precision} & \textbf{Overhead} \\
\midrule
\textbf{HAC} & \textbf{82.4} & \textbf{100.0\%} & \textbf{Minimal} \\
CluStream (MOA) & 3,854.2 & 82.5\% & High (Init) \\
ClusTree (MOA) & 2,510.8 & 78.1\% & Moderate \\
\bottomrule
\end{tabular}
}
\end{table}
...
We validated live resilience by simulating a camera outage: at frame 180, an event disabling Camera 30's links was injected without stopping the pipeline; the control loop redeployed affected queries in \textbf{337~ms}, with recovery at frame 360 in \textbf{190~ms}---both negligible relative to the 3-second synchronisation window. Fig.~\ref{fig:reconfig_before_after} visualises the resulting topology transition on the actual floor plan.
...
\vspace{2pt}
\parbox{\linewidth}{\raggedright\footnotesize Note: parenthetical labels denote the synchronization window at which the active identity set was observed.}
\end{table}
...
The timing decomposition shows that reconfiguration overhead is dominated by query compilation, not state: the initial deployment of the eight aggregation queries took 1{,}759~ms (including JVM warm-up and first EPL compilation), while subsequent single-group topology merges completed in 170--191~ms each.

The same event-driven pattern extends beyond topology to the algorithmic parameters of the tracker. \texttt{simTh}, \texttt{epsilonMcpt}, and \texttt{shortTrackTh} are stored in an \texttt{MCPTConfigTable} and updated reactively by streaming \texttt{MCPTConfig} events; the MCPT kernel re-reads fresh values on every window call. To demonstrate this, we injected three parameter changes at frame 270---tightening the similarity threshold \texttt{simTh} from 0.75 to 0.85, reducing the clustering radius \texttt{epsilonMcpt} from 0.37 to 0.30, and raising the short-track pruning threshold \texttt{shortTrackTh} from 0 to 1---all applied with no restart, no code modification, and no loss of accumulated tracking state, effective from the very next synchronisation window. This capability demonstration shows that the streams-and-tables substrate supports operational tuning without downtime, complementing the topology latency results.
...
Occlusion handling is layered across the pipeline. Intra-camera occlusion is absorbed by the 3-second SCT window: a temporarily occluded person's tracklet remains active within the window, and visibility filtering rejects heavily occluded boxes before representative selection by requiring a minimum number of high-confidence anatomical keypoints (\texttt{keypointConditionTh}). Cross-camera occlusion and temporary disappearance are handled by the \texttt{GlobalIDTable}: un-matched identities persist for up to 120~s and re-associate on reappearance through the same topology-gated Re-ID path, while a priority-ordered timer emits \texttt{TrackExpiredEvent} to evict stale tracks. The camera-outage experiment of Section~\ref{sec:evaluation} exercises precisely this mechanism: identities 1--6 remained resident across the outage window and re-associated unchanged after recovery---the state-continuity behaviour required to bridge an occluded interval. Predictive interpolation through occluded zones via CEP patterns is future work.

\subsection{Operational Advantages}
Queryable state lets operators audit identity assignments via standard SQL in real time, and the architecture suggests a natural edge--cloud partitioning. The single-camera tracking stage runs independently per camera and emits one compact \texttt{SingleCameraResult} event per synchronisation window---replacing raw video with a low-bandwidth feature stream---while the topology-aware aggregation and global clustering are centralised in the CEP engine. Our measurements support the feasibility of this split: the association-clustering stage processes a 4-camera window in $129.27 \pm 92.49$~ms on average (streaming) versus 1{,}265~ms per-window for the batch baseline, and a topology-aware group averages $9.15 \pm 8.27$~ms in steady state (Table~\ref{tab:9camera_performance}), so modest centralised compute can absorb the association workload of many edge cameras.

Two limitations remain: the evaluation is single-node, with physically distributed edge--cloud validation as future work, and topological gating restricts \emph{where} matches are sought but not \emph{how} ambiguous appearances are disambiguated in heavily crowded scenes.

% -------------------------------------------------------------------------
\section{Conclusion and Future Work}
\label{sec:conclusion}
We presented a streams-and-tables re-architecting of Multi-Camera Object Tracking on an embeddable CEP engine, converting monolithic pipeline state into queryable relations and cross-camera matching into topology-gated stream joins. This achieves a \textbf{9.8$\times$} end-to-end speedup, a \textbf{4.16$\times$} topology-aware latency reduction, and sub-second zero-downtime reconfiguration while maintaining 100\% functional output parity. Future work includes multi-hop topological joins and physical edge-cloud deployment.
