...
\textbf{Contributions:} (1) A streams-and-tables re-architecting of MCOT as an operator graph over detection streams and relational state tables; (2) Topology-aware candidate pruning reducing evaluations from $\binom{9}{2}=36$ to $6$ pairs (\textbf{4.16$\times$} latency reduction); (3) Five-stage parity verification against a JVM batch baseline on Woven WTS, confirming 100\% Global ID parity alongside a \textbf{9.8$\times$} overall speedup; and (4) An event-driven control plane enabling zero-downtime topology reconfiguration with sub-second recovery (190--337~ms) and total state preservation.
...
1) \textbf{Ingestion and Intra-Camera Overlap Suppression (S2S):} Per-camera detection streams undergo Non-Maximum Suppression (NMS) at the ingestion boundary to eliminate spatially and temporally overlapping duplicate bounding boxes. This within-camera suppression is distinct from cross-camera field-of-view (FoV) overlap, a spatial relation between cameras modeled in the \texttt{CameraTopologyTable}.

2) \textbf{Windowed Single-Camera Tracking (S2R):} Local frame detections are grouped into tracklet segments over a 3-second sliding window (90 frames at 30~fps, \texttt{win:length\_batch(90)}), outputting structured single-camera result events.
...
5) \textbf{Global Identity Maintenance (S/R2R and R2S):} Identity mappings are materialised in a persistent relational \texttt{GlobalIDTable}; derived cluster events trigger atomic \texttt{on-merge} upserts updating last-seen timestamps and coordinates. Priority-ordered timer queries evict stale tracks (unseen for $>120$~s), emitting a \texttt{TrackExpiredEvent} (R2S) before an R2R row deletion garbage-collects heavy feature tensors.
...
\begin{figure}[t!]
    \centering
    \includegraphics[width=0.7\linewidth]{Evidence/map_topology.png}
...
All experiments ran on an Intel Core i7-10610U CPU (4 cores, 8 threads, 1.80~GHz), 32~GB RAM, 64-bit Windows 11 Pro, OpenJDK 21 (JBR-21.0.8), with JVM heap capped at \texttt{-Xms256m -Xmx256m} to simulate resource-constrained edge gateways and Esper 9.0.0 as the CEP engine. Datasets: (1) Woven VisionAI WTS (Scene 2, 4 cameras) for parity and stage speedups; (2) NVIDIA SmartSpaces (9 cameras) for topological pruning and live reconfiguration.

\subsection{RQ1: Five-Stage Implementation Parity and Speedup}
To verify that our architectural re-engineering preserves tracking accuracy, we conducted a \textbf{five-stage parity evaluation} against a JVM-native batch baseline. Because the cited OSC pipeline~\cite{yoshida2024overlap} is Python, we ported it to this functionally equivalent JVM baseline---validated on identical inputs before benchmarking (ARI = NMI = 1.0000; similarity-matrix MAD $1.63\times10^{-8}$):
\begin{enumerate}
...
We validated live network resilience by simulating a camera outage on the 9-camera layout. At frame 180, an event disabling Camera 30's links was injected without stopping the pipeline; the control loop recomputed connected components and redeployed affected queries in \textbf{337~ms}, with recovery at frame 360 in \textbf{190~ms}---both negligible relative to the 3-second synchronisation window.
...
Visual occlusion caused by obstacles or crowds is a major bottleneck in MCOT. In \textbf{STREAMTRACK}, short-term intra-camera occlusions are cushioned by the 3-second SCT window, while keypoint confidence filters reject occluded bounding boxes. For long-term cross-camera occlusions, our model provides two architectural mechanisms:
\begin{enumerate}
...
\begin{itemize}
    \item \textbf{Inspectable Identity State:} Operators can query \texttt{GlobalIDTable} via standard SQL to audit assignments and trace trajectories in real time.
    \item \textbf{Edge-Cloud Partitioning Feasibility:} Per-camera SCT can run on edge devices, emitting compact \texttt{SingleCameraResult} events over low-bandwidth links to a centralized CEP coordinator averaging only $9.15$~ms per group.
\end{itemize}

Two limitations remain: the evaluation is single-node by design, with distributed edge--cloud validation as future work, and topological gating restricts \emph{where} matches are sought but not \emph{how} ambiguous appearances are disambiguated, so heavily crowded scenes can still produce similarity ambiguities.
