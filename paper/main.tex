...
This turns each difficulty into a property of the design: topology-pruned association (scalability), an inspectable identity view (observability), and topology-as-data (zero-downtime reconfigurability). Realised on an embeddable complex-event-processing engine, the architecture reproduces the global-identity assignments of a state-of-the-art batch baseline while delivering a \textbf{9.7$\times$} overall speedup (up to \textbf{236$\times$} in the clustering stage) over an equivalent JVM-native batch implementation---eliminating the language bias of prior cross-language comparisons---with a further \textbf{4.16$\times$} latency reduction (up to \textbf{4.27$\times$} in steady state excluding JVM warm-up) from topology-aware partitioning at nine cameras.
\end{abstract}
...
\begin{itemize}[leftmargin=*]
    ...
    baseline while delivering a \textbf{9.7$\times$} overall speedup (up to \textbf{300$\times$} in
    representative selection and \textbf{236$\times$} in clustering) over an equivalent JVM batch
    baseline---eliminating the language bias of prior cross-language comparisons---with a further
...
Implementing MCOT via SQL/EPL on a complex-event-processing engine such as Esper~\cite{espertech2024} offers four systems-level advantages over conventional distributed stream processors:
\begin{itemize}[leftmargin=*]
    \item \textbf{Embeddable JVM-local runtime vs.\ distributed clusters.} Distributed frameworks such as Apache Flink~\cite{carbone2015flink} or Spark Streaming run heavy external clusters and incur network serialisation latency when passing high-dimensional visual feature tensors across node boundaries. Esper is a lightweight embeddable library running inside the application process, enabling zero-copy sharing of appearance tensors between tracking operators and relational state in memory.
    \item \textbf{Relational state-table semantics vs.\ stream-only systems.} Log-processing systems such as Apache Kafka~\cite{kreps2011kafka} offer only basic state abstractions and lack first-class SQL-like tables with primary keys, indexes, and continuous transactional merges. Esper provides full relational tables, letting us model the active camera neighbourhood and global identity mappings as queryable relations that can be inspected at runtime to audit association decisions.
    \item \textbf{Deterministic memory and tensor management.} Whereas Flink CEP stores state in opaque backends (e.g., RocksDB), Esper allows priority-annotated temporal queries (e.g., \texttt{TrackExpiredEvent}) that evict stale \texttt{GlobalIDTable} rows and trigger immediate JVM garbage collection of heavy vision tensors after a configurable inactivity window.
    \item \textbf{Zero-downtime topology reconfiguration.} In Flink or Spark, changing the join topology requires stopping the job and redeploying from a savepoint. Esper compiles, deploys, and undeploys EPL statements at runtime, so our reactive loop installs updated topological joins on-the-fly when the \texttt{CameraTopologyTable} changes, without interrupting tracking.
\end{itemize}
...
\begin{table}[H]
    \centering
    \caption{Architectural Speedup (Java Batch Baseline vs. Java Streaming) for Woven WTS Dataset (25-Window Average, 4 cameras, Scene~2).}
    \label{tab:performance_combined}
    \resizebox{\linewidth}{!}{
    \begin{tabular}{@{}l|rrr@{}}
        \toprule
        \textbf{Pipeline Stage} & \textbf{Batch (ms)} & \textbf{Streaming (ms)} & \textbf{Speedup} \\ \midrule
        Measure World Coordinates & 8.65 & 1.25 & \textbf{6.9$\times$} \\
        Representative Selection & 10235.16 & 34.06 & \textbf{300.5$\times$} \\
        Re-ID Similarity Matrix & 65.08 & 3.67 & \textbf{17.7$\times$} \\
        Matrix Zeroing & 6.09 & 0.03 & \textbf{214.4$\times$} \\
        Similarity Replace (World) & 0.06 & 0.00 & \textbf{25.0$\times$} \\
        Clustering (HAC) & 20366.55 & 86.16 & \textbf{236.4$\times$} \\
        Global ID Assignment & 26.22 & 3.13 & \textbf{8.4$\times$} \\
        \midrule
        Total MCPT Core & 30707.81 & 129.27 & \textbf{9.5$\times$} \\
        Total MCPT (incl. Reassign) & 31202.58 & 129.27 & \textbf{9.7$\times$} \\
        \bottomrule
    \end{tabular}
    }
\end{table}

The streaming architecture's advantage is most pronounced in the \textbf{Representative Selection} (Stage~2, batch: 10,235~ms vs. streaming: 34~ms per window on average) and \textbf{Clustering (HAC)} (Stage~6, batch: 20,367~ms vs. streaming: 86~ms). These stages are both I/O-intensive---reading pre-computed Re-ID features from disk---and compute-intensive---iterating over all pairwise tracklets in the batch case. The streaming engine exploits incremental windowing: features are pre-loaded and maintained in stateful tables, and only the new window's deltas are processed.

The \textbf{Similarity Matrix} stage (batch: 65~ms vs. streaming: 3.7~ms, 17.7$\times$ speedup) benefits from the same incremental access pattern. The batch pipeline loads and constructs the full feature matrix for all tracklets across all windows from scratch, whereas the streaming equivalent continuously maintains the intermediate matrix representation.

Overall, the streaming architecture delivers a \textbf{9.7$\times$} end-to-end speedup (cumulative over 25 windows: 3,232~ms streaming vs. 31,203~ms batch core). These improvements stem from the architectural shift from batch-oriented processing---where each window incurs I/O and recomputation costs proportional to the full accumulated state---to incremental stream processing, where state lives in relational tables and per-window work is proportional only to newly arriving data.
...
\begin{figure}[H]
    ...
    \caption{Total MCPT Latency comparison over 25 windows. The batch baseline scales super-linearly, reaching over 6 seconds per window. In contrast, the relational streaming architecture processes the identical workload in approximately 129~ms per window on average.}
    \label{fig:total_latency}
\end{figure}

Figure~\ref{fig:total_latency} visualises the latency divergence. While the batch baseline's per-window latency grows with the accumulating tracklet count---exceeding 6~seconds at peak density---the streaming architecture maintains a stable sub-200~ms pace throughout the entire 25-window run. The cumulative speedup reaches \textbf{9.7$\times$} overall (3,232~ms streaming vs. 31,203~ms batch core), confirming that the streaming architecture neutralises the scalability bottleneck that plagues conventional batch-oriented MCPT implementations.
...
The most dramatic speedups are observed in \emph{Representative Selection} (300.5$\times$) and \emph{Clustering (HAC)} (236.4$\times$), both of which are I/O- and compute-intensive stages that benefit from the streaming engine's incremental state management. The \emph{Similarity Matrix} stage achieves a 17.7$\times$ speedup, attributed to the architectural shift from batch-oriented feature reconstruction to stream-oriented incremental matrix maintenance. Because both the batch baseline and the streaming implementation share the same JVM, math library (ND4J/OpenBLAS), and heap configuration, these gains are purely architectural---they derive from the streams-and-tables design rather than from language-level performance differences. Overall, the system processes a 4-camera window in approximately 129~ms on average (streaming) vs.~1,248~ms (batch core per-window average), demonstrating robust capability for real-time operation in diverse multi-camera environments.
