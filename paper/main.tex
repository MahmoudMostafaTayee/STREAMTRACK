...
In this section, we detail our experimental evaluation, starting with the research questions, then describing the baselines we benchmark against, the experimental setup, code porting from Python to Java, and parity checking, and finally presenting the results. The source code and instructions to reproduce the experiments are available at \url{https://github.com/MahmoudMostafaTayee/STREAMTRACK}.
...
% TODO[Student[done: the fig is actually from the woven wts dataset, so it was just a typo]]: Table~\ref{tab:parity_metrics} reports parity on Woven WTS only, yet
...
\begin{table}[H]
    ...
    \begin{tabular}{@{}l|rr|rr@{}}
        ...
        \midrule
        Total MCPT & 31620.00 & 100\% & $129.27 \pm 92.49$ & \textbf{9.8$\times$} \\
        \bottomrule
    \end{tabular}
\end{table}
% TODO[Student[done]]: confirm the batch stage decomposition above against your measurement logs,
...
%TODO[done]: In which table/figure is this reported?
%The effects of these parameter changes are demonstrated qualitatively: the pipeline accepts the new parameters on the next synchronisation window with zero downtime and no loss of accumulated tracking state. Unlike the topology reconfiguration results in Table~\ref{tab:reconfiguration_timings} and Table~\ref{tab:global_id_evidence}, the runtime parameter reconfiguration is a capability demonstration rather than a benchmark target, so no separate table or figure can even be illustrated.
