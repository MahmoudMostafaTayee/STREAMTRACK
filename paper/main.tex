...
\draw[ctrl] ($ (reconfig.west) + (-1.6,0) $) -- node[above, font=\scriptsize] {CameraTopology events} (reconfig.west);
% control wiring
\draw[ctrl, <->] (reconfig) -- node[right, font=\scriptsize, align=left] {upsert /\\watch} (topology);
\draw[ctrl] (topology) -- node[right, font=\scriptsize, align=left] {deploy /\\undeploy} (join);
\draw[ctrl] (configtable.south) .. controls +(0,-0.6) and +(-1.2,0.8) .. node[below, font=\scriptsize, sloped, pos=0.6] {reads params} (mcpt.north west);
% ---------------- Data-plane arrows ----------------
\draw[stream] ($ (ingest.west) - (1cm,0) $) -- node[above] {\scriptsize Detections} (ingest.west);
\draw[arrow] (ingest) -- node[above] {\scriptsize stream} (scpt);
\draw[arrow] (scpt) -- node[above, align=center] {\scriptsize SingleCam\\ \scriptsize Result} (join);
...
\texttt{MCPTConfig} events upsert into control tables, and a reactive reconfiguration loop
watches the \texttt{CameraTopologyTable}, recomputes connected components, and deploys or
undeploys association queries at runtime---so both the network topology and the algorithmic
parameters are reconfigurable as data, with zero downtime.}
\label{fig:architecture}
\end{figure*}
...
upsert into control tables, and a reactive reconfiguration loop watches the topology relation,
recomputes the connected components of the neighbourhood graph, and redeploys the affected
association queries at runtime. Operational concerns---which cameras are grouped, which
thresholds apply, when queries are rebuilt---are thus expressed in the same streams-and-tables
vocabulary as the tracking itself, rather than in configuration files or code. The data plane
...
% TODO[Student]: verify the control-plane wiring against the implementation:
...
%TODO[Student]: Put proper EPL staements here.
\begin{verbatim}
...
A subtlety of this design deserves emphasis. The deployed query above contains no runtime join against the \texttt{CameraTopologyTable}; its neighbour set appears as a compiled \texttt{IN(\dots)} predicate. This is deliberate: the stream--table join is evaluated at \emph{plan time} rather than per tuple. Whenever the topology changes, the reactive
reconfiguration loop (Section~\ref{sec:multi:topology}) joins the current \texttt{CameraTopologyTable} against the set of active aggregation queries, computes the connected components of the neighbourhood graph, and rewrites and redeploys the affected
queries with their neighbour sets baked in as membership predicates. The join result is thus materialised into the query plan once per topology change, instead of being re-evaluated for
every arriving \texttt{SingleCameraResult} event. This is the streaming analogue of partition pruning and predicate pushdown in relational query optimisation: because camera topology
changes orders of magnitude less frequently than detections arrive, folding the join into the plan eliminates a per-tuple table probe on the hot path while preserving the relational
semantics; the topology relation remains the single source of truth, and every deployed predicate is derivable from it. The same mechanism is what makes reconfiguration zero-downtime:
a topology update is a plan change, not a code change.
...
\todo{In the figure, the dots for cameras 17 and 13 are indistinguishable}
\begin{figure}[htb!]
    \centering
...
\begin{itemize}[leftmargin=*]
    ...
    % TODO[Student]: if a cameras-vs-latency scaling curve (e.g., topology-aware latency at
...
%TODO[Student]: please fix/add these labels
...
% TODO[Student]: insert the Python-vs-Java port parity numbers you produced during
