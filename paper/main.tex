...
This turns each difficulty into a property of the design: topology-pruned association (scalability), an inspectable identity view (observability), and topology-as-data (zero-downtime reconfigurability). Realised on an embeddable complex-event-processing engine, the architecture reproduces the global-identity assignments of a state-of-the-art batch baseline while delivering a \textbf{9.7$\times$} overall speedup (up to \textbf{236$\times$} in the clustering stage) over an equivalent JVM-native batch implementation---isolating the architectural speedup from language, runtime, and library effects---with a further \textbf{4.16$\times$} latency reduction (up to \textbf{4.27$\times$} in steady state excluding JVM warm-up) from topology-aware partitioning at nine cameras.
\end{abstract}
