...
GROUP_COLORS = {
    "g1": {
        "fill": (0, 100, 255, 60),      # Blue cone (Group 1 / Merged)
        "outline": '#0064FF',
        "text": '#002E7A',
    },
    "g2": {
        "fill": (0, 200, 50, 60),       # Green cone (Group 2)
        "outline": '#00C832',
        "text": '#005A16',
    },
    "g3": {
        "fill": (255, 140, 0, 60),      # Orange cone (Group 3)
        "outline": '#FF8C00',
        "text": '#A35600',
    },
    "g4": {
        "fill": (160, 32, 240, 60),     # Purple cone (Group 4)
        "outline": '#A020F0',
        "text": '#5A0080',
    },
    "gray": {
...
def generate_topology_map(filename, groups, outlier_group, draw_connections=True):
    ...
    for cam in cam_data:
        ...
        if is_grouped:
            ...
            # Gray out cameras not in the active experiment to focus viewer attention
        ...
        if is_important:
            ...
            if u > width/2: text_offset_x = -100
            if v > height/2: text_offset_y = 30

            label_pos = (u + text_offset_x, v + text_offset_y)
            draw_label(draw, label_pos, short_name, text_color)
    ...
    out_path = os.path.join("c:\\OURs\\Thesis\\Real-Time_Multi-Camera_People_Tracking_using_Event_Stream_Processing\\paper\\Evidence", filename)
    final_img.convert('RGB').save(out_path)
    print(f"Saved {filename} to {out_path}")

if __name__ == "__main__":
    print("Generating Pre- and Post-Reconfiguration topology maps...")
    
    # 1. Pre-Reconfiguration Map (Windows 0-1)
    # Groups: Group 1 (2,17), Group 2 (1,11), Group 3 (19,27), Group 4 (21,25,30)
    pre_groups = {
    ...
    generate_topology_map("map_topology_pre_reconfig.png", pre_groups, [], draw_connections=True)

    # 2. Post-Reconfiguration Map (Windows 2-5)
    # Groups: Group 1 (2,17,19,27) [Merged!], Group 4 (21,25,30) [Unchanged!], Cams 01 & 11 are standalone (separated!)
    post_groups = {
        "g1": ['Camera_02', 'Camera_17', 'Camera_19', 'Camera_27'], # Merged
        "g2": ['Camera_01'], # Independent
        "g3": ['Camera_11'], # Independent
        "g4": ['Camera_21', 'Camera_25', 'Camera_30'] # Unchanged
    }
    generate_topology_map("map_topology_post_reconfig.png", post_groups, [], draw_connections=True)
    print("Done!")
